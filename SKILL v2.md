...
[4.5] VISUAL & LINKS    → data charts (matplotlib) + hyperlinks on every source citation
[5] SEO + GEO           → on-page checklist + AI-bot-friendly formatting
[6] QA REPORT           → rubric score + publish verdict
```

Enter at any stage. Run all 8 if starting from scratch with a spec + keyword. Start at Stage 5 if given an existing draft to optimize. Run Stage 4.5 standalone if given a finished draft that needs charts and source links added.
...
Runs automatically after Stage 4. Produces two outputs: (1) data charts embedded in the document, (2) all source citations converted to live hyperlinks.

### Charts

**Qualifying criteria** — generate a chart if the data point meets any of:
- Change over time (2+ time periods)
- Comparison between 2+ assets or metrics
- Trend or cumulative flow (inflows/outflows)
- Scored index or rating
- Ratio that changed over time
- Series of 3+ values

**Chart type selection:**
- Performance over time → line chart with fill-under
- Inflows/outflows → bar chart (green = positive, red = negative)
- Index/score over time → bar chart with color-coded zones
- Ratio over time → line or area chart
- Single-point comparison → stat callout box

**Design standards (all charts):**
- Background: `#F7F7F7`, no heavy gridlines
- Primary brand color: `#E8650A` (crypto/Web3 default)
- Remove top + right spines
- Annotate key data points directly on chart (peaks, troughs, thresholds)
- Source line: gray italic at bottom-left — `Source: [Name] | [Notes]`
- Font: DejaVu Sans, 150 DPI
- Maximum 6 charts per article

**For each chart:** generate PNG via Python + matplotlib → embed in doc → write 1–2 sentence caption ("Chart N: [what it shows]. Sources: [names].") → write alt text string.

**Placement:** immediately after the paragraph that first references the data — never before, never stacked back-to-back.

**No qualifying data:** output a styled stat callout box instead.

---

### Hyperlinks

**Find all inline citations** matching these patterns:
- `— [Source, Year]` / `per [Source]` / `according to [Source]`
- `[Source] reports` / `[Source] analysis` / `([Source])`

**For each citation:**
1. Web search to find the exact article: `[publication] [topic] [date]`
2. Verify URL resolves
3. Replace plain-text mention with clickable hyperlink, anchor = source name
4. If exact article not found → link to publisher homepage + flag `[VERIFY URL]`
5. Unattributed claims → flag `[SOURCE NEEDED]`, do not invent a source

**Link rules:** prefer original publisher over aggregators; never link paywalled if free version exists.

---

### Output Summary

Append to draft after completion:
...
Charts generated:      [N] / [max 6]
  - Chart 1: [title] → placed after [section]
  [...]
Source links resolved: [N resolved] / [N total citations]
  - [Source] → [URL] ✅ / ⚠️ [VERIFY URL]
Unlinked flags:        [N] → [SOURCE NEEDED items]
Alt text strings:      Chart 1: "[alt text]" / Chart 2: "[alt text]"
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
| Case | Action |
|---|---|
| No chart-worthy data | Stat callout boxes only. Flag: "No series/comparison data found." |
| >6 chart-worthy data points | Prioritize: (1) comparisons, (2) trends, (3) indexes. Rest → callout boxes. |
| URL paywalled | Search for free version. If not found: homepage link + `[VERIFY URL]` |
| Article not found | Publisher homepage + `[SOURCE NEEDED — could not verify]` |
| Vague citation ("analysts say") | Flag `[SOURCE NEEDED]` — never invent source |
| Web search off | Skip link resolution. Flag all: `[HYPERLINK NEEDED — web search off]`. Charts still generated. |
...
| Data charts embedded (Stage 4.5) | 5 |
| Source hyperlinks resolved (Stage 4.5) | 5 |
| Internal linking suggestions (min 2) | 10 |
| Readability (paragraphs ≤4 lines) | 10 |

Thresholds: 95–110 = Publish-ready | 80–94 = Minor fixes | Below 80 = Needs revision
...
╚══════════════════════════════════════════════╝
ARTICLE: [Title] | KEYWORD: [primary] | WORDS: [X] | UGC: [X sections / None] | CHARTS: [N embedded] | LINKS: [N resolved]
...
| Draft has no chart-worthy data | Output stat callout boxes. Flag: "No series/comparison data — consider adding benchmarks." |
| All source URLs paywalled | Link publisher homepages. Flag all with `[VERIFY URL]`. |
...
- Generates data charts (matplotlib PNG) — does NOT generate editorial/decorative images
